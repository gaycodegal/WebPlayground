...
AngleBro.prototype.stepTowards = function(pt, prev){
    var st = fixAngle(this.line.c.x),
	et = fixAngle(angleBetween(this.line.o, pt) - Math.PI);
    var delta = this.sign * fixAngle(et - st) / this.f1;
    var end = fixAngle(st + delta);
    //console.log();
    
    if(this.next){
	var diffNext = fixAngle(end - fixAngle(this.next.line.c.x - Math.PI)) / Math.PI * 180;
	var diffPrev = prev ? fixAngle(end - fixAngle(prev.line.c.x - Math.PI)) / Math.PI * 180 : 180;
	if(this.sign > 0 && (Math.abs(diffNext) < 90 || Math.abs(diffPrev) < 90)){
	    this.next.setOrigin(this.line.endPoint());
	    return [diffNext, diffPrev];
	}
	this.line.c.x = end;
	this.next.setOrigin(this.line.endPoint());
	return diffNext;
...
function cycle(){
    ...
    var line = new Array(bros.length);
    var prev = null;
    for(var j = 0; j < 3; ++j){
	prev = null;
	for(var i = 0; i < n; ++i){
	    line[i] = bros[i].stepTowards(pt, prev);
	    prev = bros[i];
	}
    }
    console.log(line);
